...
pub async fn update_student(
    ...
    let existing_name: Option<String> =
        sqlx::query_scalar("SELECT nome_completo FROM students WHERE id = $1")
            .bind(id)
            .fetch_optional(pool)
            .await?;
    ...
    sqlx::query("DELETE FROM student_responsaveis_legais WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_membros_familiares WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_pessoas_autorizadas WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_beneficios WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_interacao_social WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_locais_lazer WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM student_servicos_utilizados WHERE student_id = $1")
        .bind(id)
        .execute(&mut *tx)
        .await?;
...
pub async fn delete_student(pool: &PgPool, id: i64, by_email: &str) -> Result<bool, sqlx::Error> {
    ...
    let rows = sqlx::query("DELETE FROM students WHERE id = $1")
        .bind(id)
        .execute(pool)
        .await?
        .rows_affected();
...
pub async fn list_audit_events(
    pool: &PgPool,
    limit: i64,
) -> Result<Vec<StudentAuditEvent>, sqlx::Error> {
    #[derive(sqlx::FromRow)]
    struct Row {
        id: i64,
        student_id: i64,
